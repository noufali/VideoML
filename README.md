# PyTorch-Style-Transfer (modified)

This repo is a modified version of the PyTorch Implementation (https://github.com/zhanghang1989/PyTorch-Multi-Style-Transfer) of **[MSG-Net (ours)](#msg-net)** and **[Neural Style (Gatys et al. CVPR 2016)](#neural-style)**, which has been included by [ModelDepot](https://modeldepot.io/zhanghang/multi-style-generative-network-for-real-time-transfer/overview). This provides as well a [Torch implementation](https://github.com/zhanghang1989/MSG-Net/) and [MXNet implementation](https://github.com/zhanghang1989/MXNet-Gluon-Style-Transfer).
So far there is no implementation of [optical flow](https://en.wikipedia.org/wiki/Optical_flow) in this repo, the videos get generated frame by frame. See [Fast-Artistic-Videos](https://github.com/manuelruder/fast-artistic-videos) for an example implementation.
## Server and Environment Setup

Reasonable training (around 12hours for 4Epochs) and generation times (around 1frame per second) were achieved on a Paperspace P6000 instance running "ML in a Box" (Ubuntu 16.04):

| Model             | vCPU          | RAM   | GPU   | CUDA Cores  |
| ----------------- |:-------------:|:-----:|:-----:|:-----------:|
| Nvidia P6000      | 8             | 30GB  | 24 GB | 3840

Basic environment requirements: 
PyTorch 0.3.1 (Anaconda3 environment.yml file provided in this repo) 
...
* [Real-time Style Transfer using MSG-Net](#msg-net)
	- [Stylize Images/Videos/Webcam using Pre-trained Model](#stylize-images-using-pre-trained-msg-net)
...
0. Video Demo
    * needs trained model on your style (change this in `--model` - arguments, otherwise use default): 3 already trained models are in this repo under 'models'
    * stylizes video frames by frame (without optical flow)
    * change line 25 in `vid_demo.py` to the video you want to stylize (to_do: needs to be changed to argument ...)
    * check framerate of original video (with `ffmpeg -i name_of_your_video`) and adjust in the arguments in the ffmpeg generation later

	```bash
	cd experiments
	mkdir stylize
	cd ..
	python vid_demo.py demo --model models/21styles.model
	cd stylize
	ffmpeg -framerate 60 -i %d.jpg stylized_video_output.mp4
	```
	![](vid_style.jpg)
	
0. Test the model
    * if you're using PyTorch 0.4: comment out / uncomment lines 260-271 in main.py
